...
document.addEventListener("DOMContentLoaded", () => {
    // Unique keys for reviews and comments specific to this page
    const pageIdentifier = document.title.replace(/\s+/g, '-').toLowerCase(); // Use the page's title as an identifier
    const reviewsKey = `$planet-of-the-apes-reviews`;
    const commentsKey = `$planet-of-the-apes-comments`;

    const stars = document.querySelectorAll(".star-rating label");
    let selectedRating = 0;

    // Star Rating System: Furthest left is 1-star, rightmost is 5-stars
    ...
    reviewForm.querySelector(".submit-review").addEventListener("click", () => {
        ...
        if (name && text && selectedRating) {
            ...
            localStorage.setItem(reviewsKey, JSON.stringify(storedReviews)); // Save to localStorage
    ...
    commentForm.querySelector(".submit-comment").addEventListener("click", () => {
        ...
        if (name && text) {
            ...
            localStorage.setItem(commentsKey, JSON.stringify(storedComments)); // Save to localStorage
    ...
    function createReviewElement(review) {
        ...
        reviewElement.querySelector(".delete-review").addEventListener("click", () => {
            ...
            localStorage.setItem(reviewsKey, JSON.stringify(storedReviews)); // Update localStorage
        });
    ...
    function createCommentElement(comment) {
        ...
        commentElement.querySelector(".delete-comment").addEventListener("click", () => {
            ...
            localStorage.setItem(commentsKey, JSON.stringify(storedComments)); // Update localStorage
        });
